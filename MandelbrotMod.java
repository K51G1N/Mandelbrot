...
public class MandelbrotMod extends Applet
  ...
  { private int xsize;      // dimensions of window
    ...
    private void generateImage()
      { startTime = System.currentTimeMillis();
        ExecutorService pool = Executors.newFixedThreadPool(ysize);
		for (int i = 0; i < ysize; i += taskSize)
          { // Start thread
            Callable<byte[][]> task = new WorkerThread(i, i+taskSize);
			Future<byte[][]> res = pool.submit(task);
			//Thread t = new Thread(new WorkerThread(i, i+taskSize));
            byte[][] results = res.get();
			display(results, i);
			//t.start();
          }
        waitForResults();
